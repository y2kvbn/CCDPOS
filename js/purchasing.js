...
const Purchasing = (() => {
              ...
              ${pageItems.length ? pageItems.map((po) => `
                <tr>
                  ...
                  <td class="nowrap">
                    ...
                    ${po.status !== '已入庫' ? `<button class="btn sm" data-edit="${po.id}">編輯</button><button class="btn sm success" data-receive="${po.id}">確認入庫</button><button class="btn sm danger" data-del="${po.id}">刪除</button>` : ''}
                  </td>
                </tr>
  ...
  function openPOForm(existingPO) {
    const s = DB.state;
    ...
    if (isEdit && existingPO.status !== '待入庫') { Utils.toast('此進貨單已入庫，無法編輯', 'error'); return; }
    let items = isEdit ? existingPO.items.map((it) => ({ ...it })) : [{ productId: s.products[0].id, qty: 1, cost: s.products[0].cost, expiryDate: '' }];
    let supplierId = isEdit ? existingPO.supplierId : s.suppliers[0].id;
    let poDate = isEdit ? existingPO.date : DB.todayStr();
    let note = isEdit ? (existingPO.note || '') : '';
    let aiQuery = '';
    ...
    function bodyHtml() {
      return `
        <div class="modal-title">${isEdit ? `編輯進貨單 ${existingPO.no}` : '新增進貨單'} <button class="icon-btn" id="modal-close">✕</button></div>
        <div class="form-grid">
        ...
        <div class="modal-foot">
          <button class="btn" id="modal-cancel">取消</button>
          <button class="btn primary" id="btn-save-po">${isEdit ? '更新進貨單' : '儲存進貨單'}</button>
        </div>
      `;
    }
    ...
    function bind() {
      ...
      document.getElementById('btn-save-po').onclick = (e) => {
        if (poSaved) return;
        // 第二層防護：若編輯期間此進貨單已被其他操作標記為已入庫，拒絕覆寫，避免竄改已影響庫存的紀錄。
        if (isEdit && existingPO.status !== '待入庫') {
          Utils.toast('此進貨單已入庫，無法編輯', 'error');
          Utils.closeModal();
          return;
        }
        ...
        e.currentTarget.disabled = true;
        if (isEdit) {
          existingPO.supplierId = finalSupplierId;
          existingPO.date = date;
          existingPO.items = items.map((it) => ({ ...it }));
          existingPO.note = document.getElementById('f-note').value.trim();
          DB.save(); Utils.closeModal(); Utils.toast('已更新進貨單', 'success'); render();
        } else {
          const po = {
            id: DB.uid('PO'),
  ...
  function viewPO(po) {
    ...
    Utils.openModal(`
      ...
      <div class="modal-foot">
        ${po.status !== '已入庫' ? `<button class="btn" id="btn-edit-po">編輯</button>` : ''}
        <button class="btn primary" id="modal-close2">關閉</button>
      </div>
    `, { wide: true });
    document.getElementById('modal-close').onclick = Utils.closeModal;
    document.getElementById('modal-close2').onclick = Utils.closeModal;
    const editBtn = document.getElementById('btn-edit-po');
    if (editBtn) editBtn.onclick = () => { Utils.closeModal(); openPOForm(po); };
  }
  ...
  function bind() {
    ...
    if (tab === 'purchases') {
      ...
      // 注意：不可直接把 openPOForm 當成 handler 傳入，事件物件本身是 truthy，
      // 會讓 openPOForm(existingPO) 誤判成編輯模式，必須用箭頭函式包一層確保不帶參數呼叫。
      ...
      root().querySelectorAll('[data-edit]').forEach((b) => b.addEventListener('click', () => openPOForm(DB.state.purchases.find((p) => p.id === b.dataset.edit))));
