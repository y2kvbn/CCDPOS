...
const DB = (() => {
  ...
  function seed() {
    ...
    const products = [
      { id: uid('P'), name: '天然狗糧 5kg', category: '飼料', brand: 'Royal', barcode: '4710001000015', spec: '5kg/包', price: 1280, cost: 850, safetyStock: 5, stock: 18, enabled: true, unit: '包', photo: '', batches: [{ id: uid('B'), qty: 18, expiryDate: '2027-03-01', note: '批20260901', receivedDate: '2026-09-01' }] },
      { id: uid('P'), name: '貓咪主食罐 80g', category: '飼料', brand: 'Ciao', barcode: '4710001000022', spec: '80g/罐', price: 45, cost: 25, safetyStock: 20, stock: 60, enabled: true, unit: '罐', photo: '', batches: [{ id: uid('B'), qty: 60, expiryDate: '2026-10-05', note: '批20260901', receivedDate: '2026-09-01' }] },
      { id: uid('P'), name: '潔牙骨（大）', category: '零食', brand: 'Pedigree', barcode: '4710001000039', spec: '單支', price: 120, cost: 60, safetyStock: 10, stock: 8, enabled: true, unit: '支', photo: '', batches: [{ id: uid('B'), qty: 8, expiryDate: '2026-09-28', note: '批20260801', receivedDate: '2026-08-01' }] },
      { id: uid('P'), name: '寵物洗毛精 500ml', category: '用品', brand: 'Bio-Groom', barcode: '4710001000046', spec: '500ml/瓶', price: 350, cost: 190, safetyStock: 6, stock: 12, enabled: true, unit: '瓶', photo: '', batches: [] },
      { id: uid('P'), name: '關節保健品', category: '保健品', brand: 'NHV', barcode: '4710001000053', spec: '60顆/罐', price: 890, cost: 560, safetyStock: 4, stock: 3, enabled: true, unit: '罐', photo: '', batches: [{ id: uid('B'), qty: 3, expiryDate: '2026-09-22', note: '批20260701', receivedDate: '2026-07-01' }] },
      { id: uid('P'), name: '寵物外出提籠', category: '用品', brand: 'IRIS', barcode: '4710001000060', spec: 'M號', price: 990, cost: 650, safetyStock: 2, stock: 5, enabled: true, unit: '個', photo: '', batches: [] },
    ];
    ...
    const posOrders = [];
    const members = [
      { id: uid('M'), name: '王小姐', phone: '0912-345-678', email: '', address: '', balance: 0, points: 0, note: '', pets: [{ id: uid('PET'), name: 'Momo', type: '狗', note: '' }] },
      { id: uid('M'), name: '陳先生', phone: '0933-222-111', email: '', address: '', balance: 0, points: 0, note: '', pets: [{ id: uid('PET'), name: '豆豆', type: '狗', note: '' }] },
    ];
    const memberBalanceLogs = [];
    const memberPointLogs = [];
    const loyaltyGifts = [];
    ...
    return {
      meta: {
        ...
        // 會員集點與折抵政策：預設關閉（enabled: false），不影響現有結帳流程；
        // 店家於「設定」頁開啟後，才會在 POS 結帳與會員資料出現集點/折抵相關 UI。
        loyaltyPolicy: { enabled: false, earnAmountPerPoint: 10, redeemPointsPerDollar: 10 },
      },
      products, services, suppliers, purchases, inventoryLogs, reservations, courses, enrollments, posOrders, members, memberBalanceLogs, memberPointLogs,
      loyaltyGifts, discountCampaigns,
      productCategories,
      users,
    };
  }
  ...
  function migrate(s) {
    ...
    if (!s.memberPointLogs) {
      s.memberPointLogs = [];
    }
    if (!s.loyaltyGifts) {
      s.loyaltyGifts = [];
    }
    if (!s.discountCampaigns) {
      s.discountCampaigns = [];
    }
    if (!s.meta.loyaltyPolicy) {
      s.meta.loyaltyPolicy = { enabled: false, earnAmountPerPoint: 10, redeemPointsPerDollar: 10 };
    }
