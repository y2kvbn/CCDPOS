...
const POS = (() => {
  ...
  function allCatalogItems() {
    const s = DB.state;
    const products = s.products.filter((p) => p.enabled).map((p) => ({
      ...
      low: p.stock <= p.safetyStock,
    }));
    const services = s.services.filter((sv) => sv.enabled).map((sv) => ({
      key: 'service-' + sv.id, type: 'service', refId: sv.id, name: sv.name, price: sv.price,
      cost: sv.cost, category: sv.type, stock: null, barcode: '', unit: sv.priceUnit || '次', low: false,
    }));
  ...
  function renderCatalogGrid() {
    ...
    grid.innerHTML = items.map((i) => {
      ...
      const disabled = i.type === 'product' && i.stock <= 0;
      return `
      <button class="pos-product-card ${i.low ? 'low' : ''}" data-type="${i.type}" data-id="${i.refId}" ${disabled ? 'disabled' : ''}>
        ${i.low && i.type === 'product' ? '<span class="lowtag badge yellow">低庫存</span>' : ''}
        <div class="pname">${Utils.escapeHtml(i.name)}</div>
        <div class="pstock">${i.type === 'product' ? '庫存 ' + i.stock + ' ' + (i.unit || '') : '每' + (i.unit || '次')}${inCart ? ' · 已選 ' + inCart.qty : ''}</div>
      ...
      list.innerHTML = cart.map((c, idx) => `
        <div class="cart-row" data-idx="${idx}">
          ...
          <div class="qty-stepper">
            ...
            <input type="text" value="${c.qty}" data-act="set" inputmode="numeric">
