...
const Orders = (() => {
  ...
  function reverseOrder(order, newStatus) {
    ...
    try {
      ...
      // 若此訂單曾核發集點或用點數折抵，退款/作廢時扣回已核發的點數、退回已折抵的點數（會員若已被刪除則略過）。
      if (order.memberId && ((order.pointsEarned || 0) > 0 || (order.pointsRedeemed || 0) > 0)) {
        const member = DB.state.members.find((m) => m.id === order.memberId);
        if (member) {
          if (order.pointsEarned > 0) {
            member.points = Math.max(0, (member.points || 0) - order.pointsEarned);
            DB.state.memberPointLogs.push({
              id: DB.uid('MPL'), memberId: member.id, type: newStatus === '已退款' ? '退款扣回' : '作廢扣回', points: -order.pointsEarned,
              pointsAfter: member.points, note: order.no, operator: DB.getCurrentUser().name, datetime: new Date().toISOString(),
            });
          }
          if (order.pointsRedeemed > 0) {
            member.points = (member.points || 0) + order.pointsRedeemed;
            DB.state.memberPointLogs.push({
              id: DB.uid('MPL'), memberId: member.id, type: newStatus === '已退款' ? '退款退回' : '作廢退回', points: order.pointsRedeemed,
              pointsAfter: member.points, note: order.no, operator: DB.getCurrentUser().name, datetime: new Date().toISOString(),
            });
          }
        }
  ...
  function openDetail(order) {
    ...
    Utils.openModal(`
      ...
      ${order.discountReason ? `<div class="small muted">折扣原因：${Utils.escapeHtml(order.discountReason)}${order.discountCampaignName ? ` · 優惠活動：${Utils.escapeHtml(order.discountCampaignName)}` : ''}${order.discountApprovedBy ? ` · 管理員授權：${Utils.escapeHtml(order.discountApprovedBy)}` : ''}</div>` : ''}
      ${order.pointsRedeemed ? `<div class="small muted">點數折抵：${order.pointsRedeemed} 點</div>` : ''}
      ${order.pointsEarned ? `<div class="small muted">本筆核發點數：${order.pointsEarned} 點</div>` : ''}
