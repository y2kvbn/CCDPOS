...
const Utils = (() => {
  ...
  /**
   * 虛擬數字鍵盤：平板為主要操作裝置時，避免另外接實體鍵盤或叫出系統小鍵盤。
   * 疊加於畫面最上層（同 confirmModal 的做法），可在任何已開啟的 Modal 之上使用
   * （例如結帳視窗內的「本次付款金額」）。
   *
   * 用法（非同步，callback style）：
   *   Utils.openNumpad(currentValue, (num) => { ...使用者按下確認後才執行... }, { title, allowDecimal, min, max });
   */
  function openNumpad(initialValue, onConfirm, opts = {}) {
    const title = opts.title || '輸入數字';
    const allowDecimal = opts.allowDecimal !== false;
    const min = opts.min !== undefined && opts.min !== null ? opts.min : null;
    const max = opts.max !== undefined && opts.max !== null ? opts.max : null;
    let value = (initialValue === undefined || initialValue === null || initialValue === 0) ? '' : String(initialValue);
    // 若有預填值，第一次按下數字/小數點時應該視為「重新輸入」直接取代，而不是累加在舊值後面
    // （否則畫面顯示「1」時想改成「2」，按下 2 會變成「12」）；按下清除鍵則不受此限制。
    let freshStart = value !== '';

    const overlay = document.createElement('div');
    overlay.setAttribute('data-numpad-overlay', '');
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(30,26,20,0.55);z-index:320;display:flex;align-items:center;justify-content:center;padding:20px;';
    document.body.appendChild(overlay);
    ...
    function keysHtml() {
      ...
      return digits.map((k) => `<button type="button" class="numpad-key" data-key="${k}">${k}</button>`).join('')
        + (allowDecimal ? '<button type="button" class="numpad-key" data-key=".">．</button>' : '<span></span>')
        + '<button type="button" class="numpad-key" data-key="0">0</button>'
        + '<button type="button" class="numpad-key" data-key="back">⌫</button>';
    }

    function render() {
      overlay.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:20px;width:320px;max-width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
          <div style="font-weight:700;font-size:14.5px;margin-bottom:10px;">${escapeHtml(title)}</div>
          <div class="numpad-display">${escapeHtml(value === '' ? '0' : value)}</div>
          <div class="numpad-grid">${keysHtml()}</div>
          <div class="btn-row" style="margin-top:12px;">
            <button type="button" class="btn" id="btn-numpad-cancel">取消</button>
            <button type="button" class="btn" id="btn-numpad-clear">清空</button>
            <button type="button" class="btn primary" id="btn-numpad-ok" style="flex:1;">確認</button>
          </div>
        </div>
      `;
      overlay.querySelectorAll('.numpad-key').forEach((b) => { b.onclick = () => pressKey(b.dataset.key); });
      overlay.querySelector('#btn-numpad-cancel').onclick = cleanup;
      overlay.querySelector('#btn-numpad-clear').onclick = () => { value = ''; freshStart = false; render(); };
      overlay.querySelector('#btn-numpad-ok').onclick = () => {
    ...
    function pressKey(k) {
      if (k === 'back') {
        ...
        if (freshStart) { value = ''; freshStart = false; }
        if (k === '.') { if (!value.includes('.')) value += (value === '' ? '0.' : '.'); }
        else value += k;
      }
      render();
    }
  ...
  /**
   * 把一個數字輸入框改為「唯讀 + 點擊叫出虛擬數字鍵盤」，取代原生鍵盤/系統小鍵盤。
   * 確認後會對該輸入框 dispatch input 與 change 事件，讓既有的事件綁定（無論監聽哪一種）都能正常運作。
   */
  function bindNumpadInput(inputEl, opts = {}) {
    if (!inputEl) return;
    inputEl.readOnly = true;
    inputEl.style.cursor = 'pointer';
    inputEl.addEventListener('click', () => {
      openNumpad(inputEl.value, (num) => {
        inputEl.value = num;
  ...
  return { money, fmtDate, fmtDateTime, escapeHtml, toast, confirmModal, openNumpad, bindNumpadInput, openModal, closeModal, el, debounce, bindSearchInput, downloadCSV, readImageAsDataURL, paginate, paginationHtml };
})();
